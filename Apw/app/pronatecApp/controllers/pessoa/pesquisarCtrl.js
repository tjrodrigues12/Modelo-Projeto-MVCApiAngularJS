pessoaMdl.controller('pesquisarCtrl', ['$scope', 'pessoaSvc', 'globalSvc', 'uiGridConstants', '$filter',
    function ($scope, service, globalSvc, uiGridConstants, $filter) {
        ...
        $scope.dados.grid.columnDefs = [
            ...
            { field: 'dataNascimento', displayName: 'Data de Nascimento', cellFilter: 'data', width: '15%', disableHiding: true, cellTooltip: function (row, col) { return row.entity[col.field] } },
            { field: 'sexo', displayName: 'Sexo', cellFilter: 'sexo', disableHiding: true, cellTooltip: function (row, col) { return row.entity[col.field] } },
        ...
        $scope.obterPessoaGridDados = function () {
            $scope.loadTela = service.obterPessoaGridDadosParaPesquisar($scope.dados.filtro).then(function (response) {
        ...
        $scope.editar = function (item) {
            $scope.$parent.pessoaId = item.pessoaId;
        ...
        $scope.confirmarExcluir = function (item) {
            $scope.loadTela = service.excluir($scope.$parent.pessoaId).then(function (response) {
                if (response.data)
                    $scope.obterPessoaGridDados();
            });
        }
