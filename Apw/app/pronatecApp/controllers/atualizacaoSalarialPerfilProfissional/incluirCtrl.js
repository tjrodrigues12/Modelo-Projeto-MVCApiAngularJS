atualizacaoSalarialPerfilProfissionalMdl.controller('incluirCtrl', ['$scope', 'atualizacaoSalarialPerfilProfissionalSvc', 'globalSvc', 'ValidationService',
                ...
                if (formValido) {
                    ...
                    $scope.loadSalvar = service.incluir(trabalho).then(function (response) {
                        if (response.data == true) {
                            $scope.$parent.dados = null; //Limpar $parent.dados para quando voltar a página de consulta de vínculos trabalhistas, ela consultar o novo vinculo de trabalho cadastrado
                            $scope.redirecionarPesquisar();
                        }
                    });
                }
