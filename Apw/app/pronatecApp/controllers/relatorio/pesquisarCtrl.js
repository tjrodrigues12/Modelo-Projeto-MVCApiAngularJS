relatorioMdl.controller('pesquisarCtrl', ['$scope', 'relatorioSvc', 'globalSvc', 'uiGridConstants', '$filter',
    function ($scope, service, globalSvc, uiGridConstants, $filter) {
        ...
        $scope.obterVinculosGridDados = function () {
            $scope.loadTela = service.obterVinculosGridDados($scope.dados.filtro).then(function (response) {
        ...
        $scope.imprimir = function (item) {
            $scope.carregando = service.gerarRelatorio(4,2018, item.vinculoId).then(function (response) {
